...
impl CrossmatchStar {
    pub fn from_hip_accessor(
        ...
        Ok(Self {
            ...
            parallax: ordinals
                .hip_parallax
                .map_or(Ok(f64::NAN), |n| accessor.read_f64(n))?,
            bt_mag: f32::NAN,
            vt_mag: f32::NAN,
            epoch_ra: 1.25,
            epoch_dec: 1.25,
    ...
    pub fn score(&self, gaia_star: &GaiaStar) -> f64 {
        ...
        let plx_diff =
            if self.parallax.is_nan() || self.parallax <= 0.0 || gaia_star.parallax.is_nan() {
                100.0
            } else {
        ...
        (pm_diff + 0.02).recip()
            + (plx_diff + 0.01).sqr().recip()
            + 100.0 / mag_diff.sqr()
            + 100.0 / (dist / MAS_TO_DEG).sqr()
    }
}
