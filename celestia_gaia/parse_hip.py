...
def load_xhip() -> Table:
    ...
    with open_cds_tarfile(VIZIER_DIR / "xhip.tar.gz") as tf:
        ...
        photo_data[
            "HIP"
        ].unit = None  # for some reason it is set to parsecs in the ReadMe
...
def process_xhip() -> Table:
    ...
    xhip = join(
        xhip, sptypes, keys=["HIP"], join_type="left", metadata_conflicts="silent"
    )
...
def process_hip(data: Table) -> Table:
    ...
    data["r_gaia_score"] = np.where(
        data["parallax"] <= 0, -10000.0, data["parallax"] / data["parallax_error"]
    )

    data["r_xhip_score"] = np.where(
        data["Plx"] <= 0, -10000.0, data["Plx"] / data["e_Plx"]
    )
