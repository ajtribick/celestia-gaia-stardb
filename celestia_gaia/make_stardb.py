...
parse_spectrum_vec = np.vectorize(
    parse_spectrum, otypes=[np.uint16]
...
def estimate_magnitudes(data: Table) -> None:
    ...
    imag = data["phot_g_mean_mag"].filled(np.nan) - (
    ...
    data["e_B-V"] = MaskedColumn(
        data["e_B-V"].filled(np.sqrt(f_e_bmag**2 + f_e_vmag**2))
    )
    data["V-I"] = MaskedColumn(data["V-I"].filled(f_vmag - imag))
    data["e_V-I"] = MaskedColumn(
        data["e_V-I"].filled(np.sqrt(f_e_vmag**2 + e_imag**2))
    )
...
def load_sao() -> Table:
    ...
    with (VIZIER_DIR / "sao.readme").open("r") as readme:
        reader = WorkaroundCDSReader(
            "sao.dat", ["SAO", "HD"], [np.int64, np.int64], readme
        )
...
def merge_all() -> Table:
    ...
    data["HIP"] = np.where(
        ...
        data["HIP"],
        make_tyc(
            data["HIP"] // 1000000, (data["HIP"] // 10) % 100000, data["HIP"] % 10
        ),
    ).astype(np.uint32)

    data["bp_rp"] = data["phot_bp_mean_mag"] - data["phot_rp_mean_mag"]

    data["r_est"] = MaskedColumn(
        apply_distances(GAIA_EDR3_DIR, data["source_id"]), unit="pc"
    )
...
ROT_MATRIX = np.array(
    [[1, 0, 0], [0, COS_OBLIQUITY, SIN_OBLIQUITY], [0, -SIN_OBLIQUITY, COS_OBLIQUITY]]
)
...
def make_stardb() -> None:
    ...
    with ZipFile(
        f"{archivename}.zip", "w", compression=ZIP_DEFLATED, compresslevel=9
    ) as zf:
        contents = [
            "stars.dat",
            "hdxindex.dat",
